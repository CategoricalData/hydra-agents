...
This harness was first used with Claude Code sessions, but nothing here assumes
that: other agent frameworks (OpenAI Codex has been used effectively) slot into
the same role, briefing, and lifecycle. Where a mechanism is substrate-specific
(hooks, `tmux`, `claude-remote`), treat it as the current implementation of a
framework-neutral contract: read your inbox, keep your plan-doc current, route
questions up, never touch shared state without approval.
...
deep context for *your* issue, not the whole documentation surface.

## The lifecycle, from your side

1. **Orient.** Run the startup checklist (the project's harness guide), read your
   issue (`gh issue view <NNN>`), and write `<branch>-plan.md` at the worktree root.
...
6. **Rebase + squash + verify (on signal).** `git fetch` first (`main` moves
   ...
   and re-validate on every build target your change affects (not just a subset).
