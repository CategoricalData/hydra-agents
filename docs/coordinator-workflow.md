...
— release/high-stakes coordinators run the most capable tier, staging runs a
strong/balanced tier, simple issue agents run the plan-then-execute tier, and an
issue agent that takes on children may be escalated. Consult that table when
choosing; don't duplicate the policy here.

Mechanically: `claude-remote` takes `-m <model>` / `--model <model>` and
defaults to `opus`; `spawn-issue-worktree.sh`'s `SPAWN_MODEL` (default
`opusplan`) is how the spawning parent applies the policy. Override at spawn,
e.g. `SPAWN_MODEL=opus bin/spawn-issue-worktree.sh …` for an architecture-level
issue. Manual spawns follow the same table:
`claude-remote <type>_NNN_slug -m <model>`. (Model names are the current Claude
lineup; on another framework, map to its equivalent tiers.)
...
3. **Rebase + squash + verify** — on the coordinator's signal the agent
   ...
   and re-validates. **Validate every build target the change affects, not
   just a convenient subset** — a partial-target green is not a full green, and
   a subset-only handoff for a broadly-generated change has reddened `main`
   after landing. (In Hydra specifically this means all nine host languages,
   not just the haskell/java/python triad; your project has its own full set.)
4. **Pause to save context (terminate the process, keep the worktree)** —
   ...
   **terminate the agent's process** so it stops consuming context and
...
5. **Reopen if bugs surface** — if a defect appears (e.g. a red CI job
   traced to the change), start a **new** session for that agent and give
...
6. **Finalize the worktree** — the end-of-life step, distinct from
   ...
   - **Landed + green.** Not merely "pushed" — a push with red CI is not
     ...
     run": a fast auxiliary workflow can show green while CI still runs, and
     mistaking it for CI green has caused a premature finalize.
   ...
   needs updating), ensure the agent's process is terminated, and copy
...
1. **Wait for a safe boundary.** Never reset mid-operation (mid-build, mid-
   rebase, mid-test-run). Reset only at a natural seam: after a commit,
...
3. **`/compact` or restart.** Then either `/compact` the session, or restart
   ...
   saves a re-derivation). (`/save`, `/compact` and the restart flag are
   Claude-Code-specific; the discipline — checkpoint to the plan, then reset —
...
requirement (the harness guide's "never stop to ask whether to fix" rule), and it needs
no approval no matter how deep, pre-existing, non-CI-gated, or time-consuming the fix is.
Do **not** interrupt the autonomous run to ask the human "should I fix this, or defer it /
land anyway / is this good enough?" — that non-question wastes hours (it has, repeatedly)
and the answer is always *fix*. When a spawned agent checkpoints upward with a
fix-vs-defer question, the coordinator/staging answer is "keep fixing to green," relayed
immediately — never forwarded to the human. The genuine exception is a design decision the
code cannot resolve (two incompatible valid approaches, a real API/behavior change a human
must choose); there, propose a recommended path and keep moving unless truly blocked.
...
- **Every issue must declare a parent, except `release_*` roots.** The parent
  ...
  issue is this a child of?" *before* showing the draft. See
  [`agent-hierarchy.md`](agent-hierarchy.md#mandatory-parent-the-enforceable-rule).
- **Draft-and-show is the terminal step.** Prepare the full text including the
  ...
  only on explicit approval — per the issue-filing rule in the project's harness
  guide. Never file, close, comment, label, or re-parent on your own initiative.
  Make "show the draft" the last step of the drafting task, not one that can be
  skipped when intent seems clear.
