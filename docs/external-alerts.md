...
not an LLM session.** Typical sources:
...
expect the sender to be a script and treat the message accordingly. (A reference
watchdog implementation ships with this harness under `bin/watchdog/`.)
...
1. **Acting on a resource alert is a standing responsibility, not optional.**
   ...
   quick `free -h` / a tail of the watchdog's health log), but do not sit on a
...
2. **The response is usually fleet back-pressure.** If the alert is resource
   ...
   Concretely, the heaviest fleet load is typically **concurrent compiler-heavy
   builds** (they can each hold many GB of RSS); the single most effective lever
   is to ensure **at most one heavy build runs at a time** across the machine,
   serialized through you. If the machine does freeze despite this, recovering
   the fleet afterward is [`crash-recovery.md`](crash-recovery.md).
...
2. **A stale or missing heartbeat is the condition to respond to.** If the newest
   ...
   surface — and a stalled heartbeat is how you'd learn of a repeat before anyone
   reports it. If it has indeed crashed, recovering the fleet is
...
3. **If it isn't addressed within a reasonable interval, start pausing agents.**
   ...
   heaviest offenders first (concurrent heavy builds), then quiesce more broadly
...
## Disk pressure: reclaiming rebuildable build cache

A common dominant disk consumer is per-worktree build cache — each active
worktree can carry several GB, and with dozens of parallel worktrees the sum
(tens to over a hundred GB) can fill the shared root filesystem to 100% and block
or corrupt **every** build fleet-wide (a link step dies with `No space left on
device`). Disk-filling is one of the resource conditions the watchdog alerts on;
reclaiming that cache is the staging agent's standing remedy.

**The rule (self-scoped, pre-authorized — like the liveness back-pressure above):**

- **Safe to reclaim: the build cache of any worktree whose branch has landed to
  `main` or lives in `worktrees/closed/`.** That cache only recompiles on the next
  build; the work itself is already in `main`. Verify "landed" against `main`
  (grep the issue's resolving commit into `git log origin/main`), not by
  assumption.
- **Never delete a build cache under a live build.** Before every `rm -rf`, verify
  no active build process has that worktree as its CWD. A dormant orphaned
  compiler process (0% CPU, hours old) is not an active build, but do not delete
  out from under *any* live process — leave those for the owning session to reap.
  (Hard rule: never kill processes you do not own.)
- **Own tree: reclaim freely.** Staging may clear its own build cache any time it
  has no build in flight.
- **Sibling tree: needs the owner's explicit consent OR standing user
  authorization for landed-work caches.** Note a per-session quirk: some agent
  sessions have `rm -rf` of build dirs harness-blocked while staging's is not — so
  **staging is often the fleet's disk-reclaim hand**, executing deletions that a
  permission-blocked owner has consented to but cannot run itself. Always
  verify-no-active-build first, every tree, every time.
- **Escalate the root cause, not just the symptom.** A one-time reclaim buys
  headroom; a fleet that chronically refills the disk needs the user's call on a
  standing prune. The recommended standing policy: **periodically prune the build
  cache for every worktree whose branch has landed to `main` or is in `closed/`**
  — this structurally caps the cache total without ever touching active or
  land-queued work.
- **During a disk-full episode, hold heavy builds fleet-wide** (same back-pressure
  discipline as a stale liveness beat): tell the land-critical worker(s) to hold
  any heavy build step, reclaim safe caches, confirm `df` headroom, then broadcast
  resume. Do not grant a build slot into a near-full disk — the link step will
  fail or corrupt.

> **Hydra note:** in Hydra this cache is the per-worktree Haskell `.stack-work`
> (~5–6 GB each; ~90–100 GB aggregate across the fleet). Hydra's structural fix
> was to route target-generation through a *published* host so most worktrees no
> longer need a full local compile for that path, shrinking the aggregate at its
> source. Your project's equivalent (a language's build cache, a container layer
> store) gets the same treatment: cap it with a landed/closed prune, and prefer a
> structural fix that stops creating the cache over one that keeps deleting it.

As with all self-scoped resource actions: reclaiming disk and pausing builds is
pre-authorized and reversible; it does **not** extend to GitHub writes, pushes, or
any outward action.
