...
Autonomous development runs as a fleet of LLM agent sessions ("agents") across
one or more machines. This document defines how those agents are organized, how
that organization mirrors the GitHub issue tree, how new-issue proposals reach
the user, how each agent's initial context is scoped to its role, and the
...
> model names), treat it as the current implementation of a framework-neutral
> contract, not as part of the model.
>
> **Project-agnostic.** This harness was extracted from the Hydra project, which
> remains its first and reference consumer. Worked examples below cite real Hydra
> issues and machinery; read "Hydra" in those as "the project you are developing
> with this harness." A few examples reference Hydra-specific build topology
> (multi-host code generation) — those are labelled, and stand as illustrations
> of a *class* of concern (your project has its own equivalent), not as
> requirements.
...
**Worked example (from Hydra, 2026-07-06).** The
[#416](https://github.com/CategoricalData/hydra/issues/416) subtree ("promote the
build system into Hydra") has 15 children, coordinated on the `feature_416` side
...
dedicated coordinator once one is spawned. This is exactly the transition the
messaging/hierarchy model codifies: staging sheds its ad-hoc issue-coordination
as the issue-tree hierarchy fills in. When it happens:
...
- **Touches shared surface** others depend on (a core type, a primitive, a
...
5. **File on approval only.** Filing happens only after explicit user approval
   (the issue-filing rule in the project's harness guide). The **filer** is
   whoever holds the conversation (usually the top coordinator) — it need not be
   the drafter; the draft is a complete, parent-stamped artifact any agent can
   file faithfully, so a paused or finished leaf never blocks filing.
...
- **A standing per-prompt banner.** [`bin/claude-hooks/proposals-hook.sh`](../bin/claude-hooks/proposals-hook.sh)
  ...
  *"⚠ N issue proposal(s) awaiting approval (oldest: Nd)"* whenever
  ...
  (Both hooks are Claude-Code-specific; another runner would provide an
  equivalent standing surface or rely on the polling cadence.)
...
2. sends it up the coordinator chain (ordinary message transport, copying into
   ...
   worktree predates the proposals tooling), and
...
table governs how the spawning parent sets it. (The names below are a current
...
|---|---|---|
| **Release agent** (`release_NNN_*`) | most capable available | Root of an issue subtree; longest-horizon context; every downstream agent inherits the consequences of its coordination quality. |
| **High-stakes coordinator** (large / long-running / architecturally consequential subtree) | most capable | "High-stakes" is decided by the spawning parent or the user. |
| **Simple issue agent** (single well-scoped bug/task, implement-and-hand-off) | plan-then-execute tier | Bulk of wall time is mechanical build/regen/verify; a planning-capable model stays in the loop for planning/triage via mode-switching. |
| **Staging agent** (per-machine promotion + top-level duties) | strong/balanced | Sustained cross-machine reasoning and merge judgment. |
| **Agent needing sustained planning/analysis** beyond the simple profile | strong/balanced | At the discretion of the spawning parent or the user. |

### Model escalation when an issue agent takes on children

An issue agent's responsibilities can grow mid-life: one running the
plan-then-execute tier that discovers a *true dependency* (dependency test = yes)
has just taken on the coordinator responsibility, and coordination is exactly the
sustained-planning work that tier is under-provisioned for. Handle it as an
explicit lifecycle event, riding the
...
- **Action:** the coordinator relaying the approval has the agent `/save` its
  plan-doc (now also a coordination brief for the new child) and restart at the
  class-appropriate model.
- **Target tier:** default plan-then-execute → strong/balanced on becoming a
  parent; escalate to the most-capable tier only if the new subtree is
  high-stakes — the same "high-stakes is the parent's or user's call" rule as the
  spawn-time table. One small dependency makes a balanced coordinator, not a
  top-tier one.
...
often running a **more capable model** than its child. When that gap exists, the
review relationship is not just lifecycle bookkeeping — it is **mentoring**: the
more-capable parent sharpens the child's thinking, catches errors, and supplies
context the child couldn't have. Two disciplines make this work, and both matter:
...
Each agent role should start with **only the context its task needs** — not the
entire harness guide. Loading everything into every agent bloats context, slows
the agent, and costs tokens across a long-lived fleet. The startup reading is
scoped by role:

| Role | Needs at startup | Does *not* need |
|---|---|---|
| **Issue agent** (no children yet) | Deep project context for *its* issue (the relevant recipes, references, package docs); the agent lifecycle ([`agent-handoff.md`](agent-handoff.md)); the proposal lifecycle + dependency test from this doc | Staging internals — the promotion loop mechanics, cross-machine push protocol, WIP-gate details |
| **Issue agent with the coordinator responsibility** (has children) | The above, plus the hierarchy + coordinator lifecycle ([`coordinator-workflow.md`](coordinator-workflow.md)) and this doc's proposal/finalize model. This reading is layered on when it takes on children — not front-loaded on a childless issue agent | Deep project internals beyond what its own issue demands |
| **Staging agent** | The promotion loop + branch-flow ([`branch-flow.md`](branch-flow.md)), cross-machine coordination (below), staging's non-issue duties, and *awareness* of the coordinator workflow (enough to hand off correctly) | The full depth of project-internal knowledge an issue agent needs — it promotes and coordinates, it does not implement core changes |

Practically, this means **the spawn briefing points each role at its scoped
reading list, not at "read the whole harness guide."** The harness guide remains
the always-loaded orientation for a human or a general session; a role-spawned
agent gets a narrower manifest seeded in its briefing (see the seeded assignment
in [`spawn-issue-worktree.sh`](../bin/spawn-issue-worktree.sh)). The governing
...
This inherits directly from the issue-filing rule in the project's harness guide
and applies without exception to every layer. "Draft the issue" means *show me*,
not *file it*.
...
CI-workflow-by-name gate, fetch-merge-before-push, tree-consistency.
...
Claude-Code-specific — another runner reads the same file its own way)
...
pretending the dependency does not exist. **Worked example (from Hydra):**
`release_508_hydra_0_17` cannot finalize Hydra 0.17 until its release-blocking
children close — the plan-doc's "release-blocking vs. deferrable" triage is
precisely this rule in action, with deferrable children re-parented to the next
release rather than blocking the current one.
...
2. **A push-announcement channel.** Before dispatching a fix for a red-CI
   ...
   double-work (two machines fixing the same red-CI bug) is a real, observed
   failure; this prevents it. The mechanism matters less than the invariant:
   *no staging agent starts a red-CI fix or a push without first checking
   whether another machine already has it in flight.* One implemented channel
   (Hydra's) is a dedicated `coordination` branch of a wiki repo — `STATE.md`
   (durable, edit-in-place) + `LOG.md` (append-only claims); a claim is a commit,
   so push-rejection gives atomic compare-and-swap semantics. (Hydra rejected a
   pinned coordination *issue* for this: unbounded comment growth, weaker claim
   atomicity, and — decisive — main-repo issue comments echo to a community chat,
   so coordination churn there would spam human developers. Pick a channel that
   is *not* echoed to a human-facing feed.)
3. **Serialize pushes; parallelize everything else.** Validation (build, test,
   cross-checks) is expensive and machine-local — let all machines do it
...
5. **Reconciliation is cross-machine.** Each staging agent reports its fleet's
   ...
   nothing on any machine is left behind, keyed on a **tree-consistency rule** —
   every active branch maps to exactly one issue and one owner, and the GitHub
   parent tree is made to match. See the cooperative reconciliation flow in
   [`coordinator-workflow.md`](coordinator-workflow.md).
...
the substrate and coordination-hygiene notes apply to everyone; the build-system
notes are for issue agents doing build-affecting work.)
...
- **`tmux send-keys` has real quirks.** Input can lag 10s+; verify a keystroke
  ...
  sitting in a session's input box (machine-generated, or a remote/mobile
  draft) may NOT submit via a local Enter — clear with `C-u`, retype with
...
- **Attention markers** appear under `~/.cache/claude-attention/` (written by
  ...
  TUI's session indicator (which goes stale). (These markers and hooks are
...
- **The CI workflow is the gate, by name.** Each push may trigger several
  workflows; `gh run list -L 1` returns whichever sorts first — often a fast
  auxiliary one showing green while the real CI still runs. This has bitten: an
  auxiliary workflow's green mistaken for CI green, causing a premature finalize.
  Key the gate on the workflow *named* CI, not on "the latest run." (Hydra's main
  CI takes ~3h15m end-to-end — a single Haskell job gates 8 language jobs — so
  Hydra does not flag a run "stuck" before ~3.5h; your project's timing differs.)

### On why review is non-negotiable
- The worst defects are *invisible to every test suite by construction*: a
  `.gitignore` pattern silently untracked a module family's generated JSON (on
  disk everywhere, absent from commits — only a fresh clone would fail); a
  per-package digest cache-hit read only one package's digest, so a *different*
  package's test change silently skipped regeneration. Neither is catchable by
  "did the tests pass"; both were caught by a coordinator diffing commit contents
  against the working tree. **Independent review is the safety net for the class
  of bug testing structurally cannot see.**
- **A good agent pushes back with evidence, and the coordinator must let it.**
  An agent's counter-evidence overturning the coordinator's wrong hypothesis,
  ending at the true root cause, is the system working, not insubordination.

### On build-system specifics that bite build-affecting work (issue agents)
> *This section is Hydra-specific — it is retained as a concrete illustration of
> the class of hazard a build-affecting change faces. Your project has its own
> equivalent gotchas; the transferable lesson is "a partial-target green is not a
> full green," not the specific Hydra facts.*
...
- **A new `hydra.lib` module must be registered in ~7 scattered places** (Haskell
  ...
  one and it compiles but fails at that host's test runtime only.
