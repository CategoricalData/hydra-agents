...
# agents/docs/cross-worktree-messages.md, but does the polling at hook-time
...
# Nothing new → silent. (`${#arr[@]}` is always defined for a declared array,
# so no `:-` default is needed — and combining the two is a bash syntax error.)
[ "${#NEW[@]}" -eq 0 ] && exit 0
